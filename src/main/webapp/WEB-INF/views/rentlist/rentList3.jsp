...
<div class="container">
	...
	<table class="table table-bordered">
		...
		<tr>
			...
			<th style="background-color: #ED7D31; font-weight: bold; text-align: center; color: white">대여인</th>
...
<sec:authorize access="hasRole('ROLE_ADMIN')">
			<th style="background-color: #ED7D31; font-weight: bold; text-align: center; color: white">관리자</th>
</sec:authorize>			
		</tr>
		<c:forEach items="${rentlists }" var="rent" varStatus="st">
			<tr>
				...
				<td style="text-align: center;">${rent.bwho }</td>
...
<sec:authorize access="hasRole('ROLE_ADMIN')">
				...
				<a href="/rentlist/find/${rent.rnum }"><button type="button" class="btn btn-primary btn-sm">상세관리</button></a>
				</td>
</sec:authorize>				
			</tr>
		</c:forEach>
	...
	<div class="d-flex justify-content-between">
		<ul class="pagination" >
			...
			<c:if test="${p.startPage >p.blockPage}">
				<li class="page-item">
				<a class="page-link" href="/rentlist/rentlist3/?pageNum=${p.startPage-p. blockPage }">이전</a></li>
			</c:if>
		...
		<!--페이지번호-->
			<c:forEach begin="${p.startPage }" end="${p.endPage }" var="i">
				...
				<c:if test="${ p.currentPage !=i}">
				<li class="page-item">
				<a class="page-link" href="/rentlist/rentlist3/?pageNum=${i }">${i }</a></li>
				</c:if>
			</c:forEach>
			...
			<c:if test="${p.endPage <  p.totPage }">
				<li class="page-item">
				<a class="page-link" href="/rentlist/rentlist3/?pageNum=${p.endPage+1 }">다음</a></li>
			</c:if>
		</ul>
		
		<form class="form-inline" action="/rentlist/rentlist3" method="get">
