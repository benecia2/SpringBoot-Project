	...
	<div class="d-flex justify-content-between">
		<ul class="pagination" >
			...
			<c:if test="${p.startPage >p.blockPage}">
				<li class="page-item">
				<a class="page-link" href="/rentlist/rentlist2/${principal.member.id}/?pageNum=${p.startPage-p. blockPage }">이전</a></li>
			</c:if>
		...
		<!--페이지번호-->
			<c:forEach begin="${p.startPage }" end="${p.endPage }" var="i">
				...
				<c:if test="${ p.currentPage !=i}">
				<li class="page-item">
				<a class="page-link" href="/rentlist/rentlist2/${principal.member.id}/?pageNum=${i }">${i }</a></li>
				</c:if>
			</c:forEach>
			...
			<c:if test="${p.endPage <  p.totPage }">
				<li class="page-item">
				<a class="page-link" href="/rentlist/rentlist2/${principal.member.id}/?pageNum=${p.endPage+1 }">다음</a></li>
			</c:if>
		</ul>
		
		<form class="form-inline" action="/rentlist/rentlist2/${principal.member.id}" method="get">
			<select name="field"   id="field" class="form-control mr-sm-1">
				<option value="bname">도서명</option>
				<option value="rentinfo">대여정보</option>
			</select>
